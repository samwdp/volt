//! Calculator plugin.
//!
//! Provides a simple expression evaluator that users can invoke from a
//! dedicated buffer.  This module intentionally depends only on the Rust
//! standard library and [`editor_plugin_api`] so it serves as the canonical
//! example of a "pure plugin" that can be added to the user library without
//! touching any editor-infrastructure crates.
//!
//! # How it fits the plugin model
//!
//! The host application ships a generic `plugin.evaluate` hook that any user
//! plugin can fire.  When the hook fires the host:
//! 1. Reads the active buffer's text and splits it at the
//!    [`plugin_hooks::EVALUATE_SEPARATOR_PREFIX`] line.
//! 2. Passes the input text and buffer kind to
//!    [`UserLibrary::handle_plugin_evaluate`].
//! 3. Replaces the output section with the returned lines.
//!
//! The calculator registers a `calculator.evaluate` command that simply emits
//! `plugin.evaluate`.  It also binds Ctrl+c Ctrl+c to that command.
//!
...
//! ```
//!
//! Everything above the `─── Output` separator is the *input area* (freely
//! editable).  After pressing Ctrl+c Ctrl+c the output section is replaced.
//!
//! # Supported syntax
//!
//! - Floating-point literals (`3.14`, `.5`, `1e-3`)
//! - Variables (`x = expr`)
//! - Arithmetic: `+`, `-`, `*`, `/`, `%`, `^` (power)
//! - Unary minus
//! - Parentheses
