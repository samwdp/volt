...
const EXTENSIONS: &[&str] = &["sh", "bash", "zsh", "ksh", "ash", "dash", "mksh"];

pub fn package() -> PluginPackage {
    common::package("bash", "Bash", EXTENSIONS, &["bash|shfmt|-w"])
}
...
mod tests {
    ...
    #[test]
    fn bash_package_registers_formatter() {
        let pkg = package();
        let formatter_details: Vec<&str> = pkg
            ...
            .collect();
        assert_eq!(formatter_details, &["bash|shfmt|-w"]);
    }

    #[test]
    fn bash_package_auto_attaches_all_extensions() {
        let pkg = package();
    ...
    #[test]
    fn bash_syntax_language_metadata() {
        let lang = syntax_language();
        assert_eq!(lang.id(), "bash");
        ...
        assert_eq!(grammar.install_dir_name(), "tree-sitter-bash");
        assert_eq!(grammar.symbol_name(), "tree_sitter_bash");
