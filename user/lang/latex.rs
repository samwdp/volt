...
const EXTENSIONS: &[&str] = &["tex", "dtx", "ins", "sty", "cls", "rd", "bbx", "cbx"];
...
/// Returns the syntax registration for the LaTeX tree-sitter language.
...
mod tests {
    ...
    #[test]
    fn latex_package_registers_formatter() {
        let pkg = package();
        let formatter_details: Vec<&str> = pkg
            ...
            .collect();
        assert_eq!(formatter_details, &["latex|latexindent|-w"]);
    }

    #[test]
    fn latex_package_auto_attaches_all_extensions() {
        let pkg = package();
    ...
    #[test]
    fn latex_syntax_language_metadata() {
        ...
        assert_eq!(grammar.install_dir_name(), "tree-sitter-latex");
        assert_eq!(grammar.symbol_name(), "tree_sitter_latex");
