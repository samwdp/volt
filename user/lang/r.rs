...
pub fn package() -> PluginPackage {
    common::package("r", "R", EXTENSIONS, &[])
}

/// Returns the syntax registration for the R tree-sitter language.
...
mod tests {
    ...
    #[test]
    fn r_package_metadata() {
        let pkg = package();
        assert_eq!(pkg.name(), "lang-r");
        assert!(pkg.auto_load());
    }

    #[test]
    fn r_package_has_no_formatter() {
        let pkg = package();
    ...
    #[test]
    fn r_package_auto_attaches_all_extensions() {
        let pkg = package();
    ...
    fn r_syntax_language_metadata() {
        ...
        assert_eq!(grammar.install_dir_name(), "tree-sitter-r");
        assert_eq!(grammar.symbol_name(), "tree_sitter_r");
