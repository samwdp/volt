...
const EXTENSIONS: &[&str] = &["lua", "rockspec"];

pub fn package() -> PluginPackage {
    common::package("lua", "Lua", EXTENSIONS, &["lua|stylua"])
}
...
mod tests {
    ...
    #[test]
    fn lua_package_registers_formatter() {
        let pkg = package();
        let formatter_details: Vec<&str> = pkg
            ...
            .collect();
        assert_eq!(formatter_details, &["lua|stylua"]);
    }

    #[test]
    fn lua_package_auto_attaches_all_extensions() {
        let pkg = package();
    ...
    #[test]
    fn lua_syntax_language_metadata() {
        let lang = syntax_language();
        assert_eq!(lang.id(), "lua");
        ...
        assert_eq!(grammar.install_dir_name(), "tree-sitter-lua");
        assert_eq!(grammar.symbol_name(), "tree_sitter_lua");
