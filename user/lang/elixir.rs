...
const EXTENSIONS: &[&str] = &["ex", "exs"];

pub fn package() -> PluginPackage {
    common::package("elixir", "Elixir", EXTENSIONS, &["elixir|mix|format"])
}
...
mod tests {
    ...
    #[test]
    fn elixir_package_registers_formatter() {
        let pkg = package();
        let formatter_details: Vec<&str> = pkg
            ...
            .collect();
        assert_eq!(formatter_details, &["elixir|mix|format"]);
    }

    #[test]
    fn elixir_package_auto_attaches_all_extensions() {
        let pkg = package();
    ...
    #[test]
    fn elixir_syntax_language_metadata() {
        let lang = syntax_language();
        assert_eq!(lang.id(), "elixir");
        ...
        assert_eq!(grammar.install_dir_name(), "tree-sitter-elixir");
        assert_eq!(grammar.symbol_name(), "tree_sitter_elixir");
