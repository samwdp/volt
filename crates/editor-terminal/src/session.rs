...
pub(crate) const DEFAULT_TERMINAL_SCROLLBACK: usize = 10_000;

pub(crate) const DEFAULT_CELL_SIZE: u16 = 1;

pub(crate) const DIM_FACTOR: f32 = 0.66;
...
pub struct TerminalLine {
    pub(crate) stream: TerminalStream,
    pub(crate) text: String,
}

impl TerminalLine {
    pub(crate) fn new(stream: TerminalStream, text: impl Into<String>) -> Self {
        Self {
            stream,
            text: text.into(),
        }
...
pub struct TerminalSession {
    ...
    pub(crate) transcript: TerminalTranscript,
}
...
impl TerminalSession {
    ...
    pub(crate) fn from_job_result(title: impl Into<String>, result: JobResult) -> Self {
...
#[derive(Clone)]
pub(crate) struct TerminalEventWake(Option<Arc<dyn Fn() + Send + Sync>>);
...
pub struct LiveTerminalConfig {
    ...
    pub(crate) on_event: TerminalEventWake,
}
...
#[derive(Clone)]
pub(crate) struct QueuedEventListener {
    pub(crate) tx: Sender<AlacrittyEvent>,
    pub(crate) on_event: Option<Arc<dyn Fn() + Send + Sync>>,
}
...
pub struct LiveTerminalSession {
    pub(crate) configured_title: String,
    pub(crate) title: String,
    pub(crate) term: Arc<FairMutex<Term<QueuedEventListener>>>,
    pub(crate) sender: EventLoopSender,
    pub(crate) event_rx: Receiver<AlacrittyEvent>,
    pub(crate) rows: u16,
    pub(crate) cols: u16,
    pub(crate) exit_code: Option<i32>,
...
pub(crate) fn append_lines(lines: &mut Vec<TerminalLine>, stream: TerminalStream, text: &str) {
    for line in text.lines() {
        lines.push(TerminalLine::new(stream, line));
    }
}

pub(crate) fn terminal_key_bytes(key: TerminalKey) -> &'static [u8] {
    match key {
...
pub(crate) fn terminal_snapshot_lines<T: EventListener>(term: &Term<T>) -> Vec<String> {
    let mut lines = Vec::new();
...
pub(crate) fn terminal_render_snapshot<T: EventListener>(
    term: &Term<T>,
    rows: u16,
    cols: u16,
...
pub(crate) fn map_terminal_cursor_shape(shape: CursorShape) -> TerminalCursorShape {
    match shape {
...
pub(crate) fn terminal_cell_text(cell: &TerminalCell) -> String {
...
pub(crate) fn default_terminal_index_color(index: usize) -> Rgb {
    match index {
...
pub(crate) fn cube_color_component(index: usize) -> u8 {
    if index == 0 {
        0
    } else {
...
pub(crate) fn default_terminal_named_color(named: NamedColor) -> Rgb {
    match named {
