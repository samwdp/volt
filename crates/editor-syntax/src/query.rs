...
pub(crate) fn intern_query_captures(
    query: &Query,
...
pub(crate) fn intern_theme_token(
    capture_name: &str,
...
pub(crate) fn append_query_source(mut source: String, extra_query: Option<&str>) -> String {
...
pub(crate) fn maybe_read_bundled_query_source(
    query_asset_root: Option<&Path>,
    language_id: &str,
    file_name: &str,
...
pub(crate) fn required_query_source(
    query_asset_root: Option<&Path>,
    language_id: &str,
    file_name: &str,
    kind_label: &str,
...
pub(crate) fn optional_query_source(
    query_asset_root: Option<&Path>,
    language_id: &str,
    file_name: &str,
    extra_query: Option<&str>,
...
pub(crate) fn resolve_query_source_from_raw(
    raw_source: &str,
    query_path: &Path,
    query_asset_root: Option<&Path>,
    file_name: &str,
...
pub(crate) fn resolve_bundled_query_source(
...
pub(crate) fn parse_query_inherits(source: &str) -> (Vec<String>, String) {
...
pub(crate) fn text_point_to_tree_sitter_point(buffer: &impl SyntaxText, point: TextPoint) -> Point {
...
pub(crate) fn tree_sitter_column_to_char_column(line: &str, byte_column: usize) -> usize {
...
pub(crate) fn desired_indent_for_loaded_language(
    language_id: &str,
...
pub(crate) fn indent_begin_applies(
    node: Node<'_>,
    line_index: usize,
...
pub(crate) fn aligned_indent_column(
    node: Node<'_>,
    line_index: usize,
    properties: &[QueryProperty],
    buffer: &impl SyntaxText,
...
pub(crate) fn line_intersects_node(node: Node<'_>, line_index: usize) -> bool {
    let start_line = node.start_position().row;
    let end_line = node.end_position().row;
    start_line <= line_index && line_index <= end_line
}

pub(crate) fn query_property_is_set(properties: &[QueryProperty], key: &str) -> bool {
    properties
        .iter()
        .any(|property| property.key.as_ref() == key)
}

pub(crate) fn query_property_value<'a>(
    properties: &'a [QueryProperty],
    key: &str,
) -> Option<&'a str> {
    properties
        .iter()
...
pub(crate) fn delimiter_column(line: &str, start_column: usize, delimiter: &str) -> Option<usize> {
...
pub(crate) fn first_content_column_after(line: &str, start_column: usize) -> Option<usize> {
    line.chars()
        .enumerate()
        .skip(start_column)
...
pub(crate) fn current_line_starts_with_token(
    buffer: &impl SyntaxText,
    line_index: usize,
    token: &str,
) -> bool {
...
pub(crate) fn line_starts_with_token_at_column(line: &str, column: usize, token: &str) -> bool {
    let tail = line.chars().skip(column).collect::<String>();
    tail.starts_with(token)
}

pub(crate) fn general_predicates_match(
    query: &Query,
    pattern_index: usize,
...
pub(crate) fn evaluate_general_predicate(
    operator: &str,
...
pub(crate) fn predicate_capture_node<'tree>(
...
pub(crate) fn predicate_capture_text(
    argument: Option<&QueryPredicateArg>,
    captures: &[tree_sitter::QueryCapture<'_>],
    buffer: &impl SyntaxText,
) -> Option<String> {
...
pub(crate) fn lua_pattern_matches(text: &str, pattern: &str) -> bool {
...
pub(crate) fn lua_match_here(text: &[u8], mut ti: usize, pat: &[u8], mut pi: usize) -> bool {
    loop {
...
pub(crate) fn lua_item_span(pat: &[u8], pi: usize) -> (usize, usize) {
    if pat[pi] == b'%' {
...
pub(crate) fn lua_item_matches(byte: u8, pat: &[u8], pi: usize, item_end: usize) -> bool {
    let ch = byte as char;
    if pat[pi] == b'.' {
        return true;
    }
...
pub(crate) fn lua_class_matches(ch: char, class: u8) -> bool {
...
pub(crate) fn lua_set_matches(byte: u8, pat: &[u8], pi: usize, item_end: usize) -> bool {
    let ch = byte as char;
    let mut i = pi + 1; // skip `[`
...
pub(crate) fn resolve_query_asset_root_from_roots(
