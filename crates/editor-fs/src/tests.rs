...
#[test]
fn discover_projects_finds_git_repositories_and_worktrees() -> Result<(), Box<dyn std::error::Error>>
{
...
#[test]
fn discover_projects_skips_missing_roots_without_failing() -> Result<(), Box<dyn std::error::Error>>
{
...
#[test]
fn project_discovery_fingerprint_change_invalidates_cache() -> Result<(), Box<dyn std::error::Error>>
{
    let _lock = discovery_test_lock();
    let _persist = begin_discovery_persist();
...
#[test]
fn project_discovery_persists_and_reseeds_cache_from_disk() -> Result<(), Box<dyn std::error::Error>>
{
    let _lock = discovery_test_lock();
    let _persist = begin_discovery_persist();
