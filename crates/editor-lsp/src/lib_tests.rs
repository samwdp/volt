...
fn language_server_spec_exposes_workspace_configuration_builders() {
    ...
    .with_workspace_configuration_settings(LanguageServerSpec::workspace_settings_object([(
        "csharp",
...
fn prepare_sessions_for_path_returns_filename_matches_without_extensions() {
    ...
    let sessions =
        must(registry.prepare_sessions_for_path(Path::new("Dockerfile"), Some(Path::new("P:\\"))));
